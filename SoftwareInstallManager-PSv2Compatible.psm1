	...
	process {
		try {
			if ($PSCmdlet.ShouldProcess($Path,'File Import')) {
				$Result = Start-Process 'reg.exe' -Args "import $FilePath" -Wait -NoNewWindow -PassThru
...
function Install-Software {
	...
	begin {
		...
		try {
			...
			$SystemTempFolder = (Get-WmiObject @WmiParams).VariableValue
	...
	process {
		try {
			...
			} elseif (Test-Path variable:\MsiInstallerFilePath) {
			...
			if (Test-Path variable:\MsiInstallerFilePath) {
				if (!$MsiExecSwitches) {
			...
			} elseif (Test-Path variable:\InstallShieldInstallerFilePath) {
				...
				$ProcessParams['ArgumentList'] = $InstallArgs
			} elseif (Test-Path variable:\OtherInstallerFilePath) {
				if (!$OtherInstallArgs) {
					$InstallArgs = ""
				} else {
